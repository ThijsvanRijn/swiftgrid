...
// LLM Chat node - OpenAI-compatible API
#[typeshare]
#[derive(Serialize, Deserialize, Debug, Clone)]
...
pub struct LlmNodeData {
    pub base_url: String,              // "https://api.openai.com/v1" or custom
    pub api_key: String,               // Can be {{$env.OPENAI_KEY}}
    pub model: String,                 // "gpt-4o", "gpt-3.5-turbo", etc.
...
async fn process_job(
    ...
    let (status, body) = match job.node {
        ...
        NodeType::Llm(data) => {
            // LLM Chat node - OpenAI-compatible API
            println!("  → LLM: model={}, messages={}, stream={}", data.model, data.messages.len(), data.stream);
            ...
            let endpoint = format!("{}/chat/completions", data.base_url.trim_end_matches('/'));
            
            // Stream progress (if streaming context available)
            if let Some(ref ctx) = stream_ctx {
                ctx.progress(&format!("Calling {} ({})...", data.model, endpoint)).await;
            }
            ...
            match response {
                Ok(resp) => {
                    ...
                    if data.stream && status_code == 200 {
                        ...
                        let body: serde_json::Value = resp.json().await.unwrap_or(serde_json::json!({"error": "Failed to parse response"}));
                        ...
                        if status_code == 200 {
                            ...
                            (status_code, Some(serde_json::json!({
                                "error": error_msg,
                                "status": status_code
                            })))
                        }
