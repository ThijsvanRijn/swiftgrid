...
<div class="relative">
  ...
  {#if data.label}
    <div class="absolute -top-5 left-3 flex items-center gap-1.5 px-2 py-0.5 bg-amber-50 border border-amber-300 border-b-0 text-[10px] font-medium text-amber-700 rounded-t-sm">
  ...
  <div class={`
      relative min-w-[200px] bg-card text-card-foreground rounded-none border transition-all duration-300 overflow-visible
      ${isRunning ? 'border-amber-500' : selected ? 'border-amber-500/50' : 'border-border/80'}
    `}
    style={`box-shadow: ${
      isRunning
    ...
    <div class="flex items-center justify-between px-3 py-2 border-b border-panel-border">
      <div class="flex items-center gap-2 min-w-0">
        ...
        {:else}
          <!-- Node icon (clock) -->
          <div class="h-6 w-6 border border-amber-200/70 bg-amber-500/10 flex items-center justify-center shrink-0">
          ...
          <!-- Type Badge -->
          <div class="h-6 px-2 flex items-center text-[10px] font-semibold uppercase tracking-wide border border-amber-200/70 bg-amber-50/80 text-amber-600 shrink-0">
            DELAY
          </div>
        {/if}
      </div>
      ...
      {#if isRunning}
        <div class="flex items-center gap-1 px-1.5 bg-amber-500/10 border border-amber-200 h-5">
      ...
      {:else if data.status === 'success'}
        <div class="flex items-center gap-1 px-1.5 bg-emerald-500/10 border border-emerald-200 h-5" title="Completed">
